...
def distort(image: np.ndarray, k: float = 0.0) -> np.ndarray:
    ...
    # If k is POSITIVE, barrel
    # If k is NEGATIVE, pincushion
    x_distorted = x_n * (1 + k * r**2)
    y_distorted = y_n * (1 + k * r**2)

    # Convert normalized coordinates back to pixel coordinates
    x_distorted = (x_distorted * (width / 2) + width / 2).astype(np.float32)
    y_distorted = (y_distorted * (height / 2) + height / 2).astype(np.float32)
